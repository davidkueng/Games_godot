...
#	Globals.shop_weapons = objects_tilemap
#	print(Globals.shop_weapons)
...
#
	...
	if coll and coll.collider.name == "Objects_TileMap":
		...
		var objects_tile_name = get_tile_name(coll, objects_tilemap)[0]
		var cell = get_tile_name(coll, objects_tilemap)[1]
		
		if objects_tile_name == "bow":
